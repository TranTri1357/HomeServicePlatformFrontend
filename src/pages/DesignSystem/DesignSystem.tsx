...
const Card = ({ children, className = "" }: { children: React.ReactNode; className?: string }) => (
  <div className={`bg-white rounded-2xl p-5 shadow-sm border border-border/50 ${className}`}>
    {children}
  </div>
);
...
function SectionColors() {
  ...
  return (
    <div className="space-y-8">
      <SectionTitle
        title="Colors"
        subtitle="Design tokens — mapped to Tailwind CSS utility classes"
      />
      {palettes.map((p) => (
        <Card key={p.name}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {semantic.map((s) => (
            <div key={s.name} className="flex flex-col gap-1.5">
              <div
                className="h-10 rounded-xl border border-black/5 shadow-sm"
...
function SectionTypography() {
  const scale = [
    ...
    },
    {
      ...
      weight: "400 Regular",
      lh: "1.6",
      sample: "Body large — default reading text",
    ...
    },
    {
      ...
      weight: "400 Regular",
      lh: "1.6",
      sample: "Body small — secondary description",
    ...
    },
    {
      ...
      weight: "400 Regular",
      lh: "1.5",
      sample: "Caption · helper text · timestamp",
    ...
    },
    {
      ...
      weight: "400 Regular",
      lh: "1.6",
      sample: "#BK001 · 0901 234 567 · 150,000đ",
      tw: "text-[13px] font-mono",
    },
  ];
  ...
  return (
    <div className="space-y-8">
      <SectionTitle
        title="Typography"
        subtitle="Inter font family — all sizes mapped to Tailwind CSS"
      />
      <Card>
          ...
          {scale.map((t) => (
            ...
            >
              ...
              <div className="flex-1">
                <p className={t.tw} style={{ lineHeight: t.lh }}>
                  {t.sample}
                </p>
              </div>
              <p className="text-[10px] text-blue-600 font-mono hidden md:block flex-shrink-0">
                {t.tw}
              </p>
            </div>
          ))}
...
function SectionIcons() {
  const iconGroups = [
    ...
    {
      name: "Service",
      icons: [Zap, Droplets, Wind, Brush, Wrench, Wrench, Package, Layers, Timer],
    },
    {
      name: "User & Auth",
      icons: [User, Users, Shield, Award, Eye, EyeOff, LogOut, Settings, Ban],
    },
    {
      name: "Communication",
      icons: [MessageCircle, Phone, Send, Paperclip, Bell, Flag, AlertCircle, Info],
    },
    {
      name: "Business",
      icons: [
          ...
          <Card key={group.name}>
            ...
            <div className="flex flex-wrap gap-3">
              {group.icons.map((Icon, i) => (
                <div
                  key={i}
                  className="w-9 h-9 bg-muted rounded-xl flex items-center justify-center hover:bg-accent transition-colors"
...
function SectionSpacing() {
  ...
  return (
    <div className="space-y-8">
      <SectionTitle
        title="Grid & Spacing"
        subtitle="8px base grid — all spacing values are multiples of 4px"
      />
      <Card>
          ...
          {spacingScale.map((s) => (
            <div key={s.tw} className="flex flex-col items-center gap-2">
              <div
                ...
                style={{ width: s.px, height: s.px, minWidth: 4, minHeight: 4 }}
              />
      ...
      <Card>
        <p className="text-xs font-bold text-foreground mb-4">
          Layout Grid (1440px Desktop — 12 col, 24px gutter)
        </p>
        <div className="flex gap-2 h-12">
          {Array.from({ length: 12 }).map((_, i) => (
            <div
              key={i}
              className="flex-1 bg-blue-100 rounded text-center text-[10px] text-blue-600 font-bold flex items-center justify-center"
            >
              {i + 1}
            </div>
          ))}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 text-xs text-muted-foreground">
          <div>
            <span className="font-bold text-foreground">Mobile:</span> 4 col, 16px margin
          </div>
          <div>
            <span className="font-bold text-foreground">Tablet:</span> 8 col, 20px gutter
          </div>
          <div>
            <span className="font-bold text-foreground">Desktop:</span> 12 col, 24px gutter
          </div>
          <div>
            <span className="font-bold text-foreground">Max width:</span> 1440px centered
          </div>
        </div>
      </Card>
    </div>
...
function SectionEffects() {
  ...
  return (
    <div className="space-y-8">
      <SectionTitle
        title="Effects & Variables"
        subtitle="CSS custom properties — theme.css tokens"
      />
      <Card>
      ...
      <Card>
        ...
        <p className="text-xs text-muted-foreground mb-4">
          Referenced via Tailwind @theme inline mapping
        </p>
        <div>
          {tokens.map((t) => (
            <div
            ...
            >
              <div
                ...
                style={{ background: t.value.startsWith("#") ? t.value : "#2563EB" }}
              />
              <p className="w-48 text-xs font-mono font-bold text-foreground flex-shrink-0">
                {t.name}
              </p>
    ...
    <div className="space-y-8">
      ...
      <Card>
        <SubSection title="Variants">
          <div className="flex flex-wrap gap-3">
            <button className="px-4 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-semibold hover:bg-blue-700 transition-colors shadow-sm shadow-blue-200">
              Primary
            </button>
            <button className="px-4 py-2.5 bg-green-600 text-white rounded-xl text-sm font-semibold hover:bg-green-700 transition-colors">
              Secondary
            </button>
            <button className="px-4 py-2.5 border-2 border-blue-600 text-blue-600 rounded-xl text-sm font-semibold hover:bg-accent transition-colors">
              Outline
            </button>
            <button className="px-4 py-2.5 bg-muted text-foreground rounded-xl text-sm font-semibold hover:bg-accent transition-colors">
              Ghost
            </button>
            <button className="px-4 py-2.5 bg-red-600 text-white rounded-xl text-sm font-semibold hover:bg-red-700 transition-colors">
              Danger
            </button>
            <button className="px-4 py-2.5 bg-amber-500 text-white rounded-xl text-sm font-semibold hover:bg-amber-600 transition-colors">
              Warning
            </button>
            <button className="px-4 py-2.5 text-blue-600 text-sm font-semibold hover:underline">
              Link
            </button>
          </div>
        </SubSection>
        ...
        <SubSection title="Sizes">
          <div className="flex flex-wrap items-center gap-3">
            <button className="px-2.5 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-semibold">
              XS · h-7
            </button>
            <button className="px-3 py-2 bg-blue-600 text-white rounded-xl text-sm font-semibold">
              SM · h-9
            </button>
            <button className="px-4 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-semibold">
              MD · h-10 ●
            </button>
            <button className="px-5 py-3 bg-blue-600 text-white rounded-xl text-base font-semibold">
              LG · h-12
            </button>
            <button className="px-6 py-3.5 bg-blue-600 text-white rounded-xl text-lg font-bold">
              XL · h-14
            </button>
          </div>
        </SubSection>
        ...
        <SubSection title="States">
          <div className="flex flex-wrap gap-3">
            ...
            <button className="px-4 py-2.5 bg-blue-700 text-white rounded-xl text-sm font-semibold ring-2 ring-blue-600 ring-offset-2">
              Focus
            </button>
            <button className="px-4 py-2.5 bg-blue-800 text-white rounded-xl text-sm font-semibold">
              Active
            </button>
            <button
      ...
      <Card>
        <div className="grid md:grid-cols-2 gap-5">
          <div className="space-y-1">
            ...
            <input
              className="w-full px-4 py-2.5 bg-muted rounded-xl text-sm border border-transparent focus:outline-none"
          ...
          <div className="space-y-1">
            ...
            <input
              className="w-full px-4 py-2.5 bg-muted rounded-xl text-sm border-2 border-blue-600 focus:outline-none ring-4 ring-blue-50"
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              <input
                className="w-full px-4 py-2.5 bg-muted rounded-xl text-sm border-2 border-green-500 focus:outline-none pr-10"
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              <input
                type="password"
                className="w-full px-4 py-2.5 bg-red-50 rounded-xl text-sm border-2 border-red-400 focus:outline-none pr-10"
                defaultValue="123"
              />
          ...
          <div className="space-y-1">
            ...
            <input
              className="w-full px-4 py-2.5 bg-muted/50 rounded-xl text-sm border border-transparent text-muted-foreground cursor-not-allowed"
              value="BK001-2026"
              disabled
            />
          </div>
          <div className="space-y-1">
            ...
            <div className="flex items-center border border-border rounded-xl overflow-hidden bg-muted">
              <span className="px-3 py-2.5 text-sm text-muted-foreground bg-muted/80 border-r border-border">
                ₫
              </span>
              <input
          ...
          <div className="md:col-span-2 space-y-1">
            ...
            <textarea
              className="w-full px-4 py-2.5 bg-muted rounded-xl text-sm border border-transparent focus:border-blue-600 focus:outline-none resize-none"
              rows={3}
        ...
        <Card>
          <SubSection title="Select / Dropdown">
            <div className="space-y-3">
              ...
              <select
                className="w-full px-4 py-2.5 bg-muted/50 rounded-xl text-sm border border-transparent text-muted-foreground cursor-not-allowed appearance-none"
                disabled
              >
          ...
          <SubSection title="Radio">
            {[
              ...
              <label key={v} className="flex items-center gap-3 py-1.5 cursor-pointer">
                <div
                  onClick={() => setRadio(v)}
                  className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors flex-shrink-0 ${radio === v ? "border-blue-600" : "border-border hover:border-blue-400"}`}
                >
          ...
          <SubSection title="Toggle Switch">
            {[
              ...
              <div key={k} className="flex items-center justify-between py-2">
                <span className="text-sm text-foreground">{label}</span>
                <button
                  onClick={() => setSw((p) => ({ ...p, [k]: !p[k as "a" | "b"] }))}
                  className={`relative w-11 h-6 rounded-full transition-colors ${sw[k as "a" | "b"] ? "bg-blue-600" : "bg-gray-300"}`}
                >
                  <div
                    className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${sw[k as "a" | "b"] ? "translate-x-5" : "translate-x-0.5"}`}
                  />
                </button>
              </div>
            ))}
          ...
          <div className="flex flex-wrap gap-2">
            {filters.map((f) => (
              <button
                key={f}
                onClick={() => setActive(f)}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${active === f ? "bg-blue-600 text-white" : "bg-muted text-muted-foreground hover:bg-accent"}`}
              >
                {f === "all" ? "Tất cả" : f}
              </button>
            ))}
        ...
        <SubSection title="Sort Controls">
          <div className="flex items-center gap-2">
            ...
            {["Giá tăng dần", "Giá giảm dần", "Lượt đặt nhiều nhất"].map((s, i) => (
              <button
                key={s}
                className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${i === 0 ? "bg-accent text-blue-600" : "text-muted-foreground hover:bg-muted"}`}
              >
                {i === 0 ? (
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-border/50 hover:shadow-md transition-shadow">
            <div className="relative">
              <img
                src="https://images.unsplash.com/photo-1621905251189-08b1489462be?w=300&h=160&fit=crop&auto=format"
                alt="service"
            ...
            <div className="p-3">
              ...
              <div className="flex gap-1.5 mt-2">
                <button className="flex-1 py-1.5 border border-blue-600 text-blue-600 rounded-lg text-[11px] font-bold">
                  Chi tiết
                </button>
                <button className="flex-1 py-1.5 bg-blue-600 text-white rounded-lg text-[11px] font-bold">
                  Đặt lịch
                </button>
              </div>
            </div>
        ...
        <div>
          ...
          <div className="bg-white rounded-2xl p-3 shadow-sm border border-border/50 hover:shadow-md transition-shadow">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-1">
                  <p className="font-bold text-sm text-foreground">Nguyễn Văn An</p>
            ...
            <button className="w-full mt-3 py-2 bg-blue-600 text-white rounded-xl text-xs font-bold hover:bg-blue-700 transition-colors">
              Đặt lịch
            </button>
          </div>
        </div>
            ...
            {[
              ...
              return (
                <div key={b.id} className="flex items-center gap-3 p-3 bg-muted rounded-2xl">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <p className="font-bold text-sm text-foreground">{b.service}</p>
                    </div>
                  ...
                  <div className="text-right flex flex-col items-end gap-1">
                    ...
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${s.color}`}
                    >
...
function SectionTables() {
  ...
  const headerCls =
    "text-left text-xs font-bold text-muted-foreground px-4 py-3 cursor-pointer hover:text-foreground";
              ...
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${s}`}>
                        {r.status === "active"
                          ? "Hoạt động"
                          : r.status === "pending"
                            ? "Chờ duyệt"
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        <button className="px-2 py-1 bg-muted hover:bg-accent rounded-lg text-xs font-semibold transition-colors">
                          Chi tiết
                        </button>
                        <button className="px-2 py-1 bg-muted hover:bg-accent rounded-lg text-xs font-semibold transition-colors">
                          <Edit3 className="w-3 h-3" />
                        </button>
                        <button className="px-2 py-1 bg-red-50 hover:bg-red-100 rounded-lg text-xs font-semibold transition-colors text-red-600">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-border">
          ...
          <div className="flex gap-1">
            {[1, 2, 3, "...", 38].map((p, i) => (
              <button
                key={i}
                className={`w-8 h-8 rounded-lg text-xs font-semibold transition-colors ${p === 1 ? "bg-blue-600 text-white" : "border border-border hover:bg-muted"}`}
              >
                {p}
              </button>
            ))}
...
function SectionBadges() {
  ...
  return (
    <div className="space-y-8">
      <SectionTitle
        title="Status Badges"
        subtitle="Pill badges with dot indicator — all system states"
      />
      <Card>
        <div className="flex flex-wrap gap-3">
          {statuses.map((s) => (
            <span
              key={s.label}
              className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${s.color}`}
            >
              <span className={`w-1.5 h-1.5 rounded-full ${s.dot}`} />
              {s.label}
            </span>
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          ...
          {[
            ...
            ["bg-gray-600 text-white", "Neutral"],
          ].map(([cls, l]) => (
            <span key={l} className={`px-3 py-1 rounded-full text-xs font-bold ${cls}`}>
              {l}
            </span>
          ))}
        </div>
      ...
      <Card>
        <div className="flex gap-3">
          <button
            ...
            className="px-4 py-2.5 bg-red-600 text-white rounded-xl text-sm font-semibold hover:bg-red-700 transition-colors"
          >
            Xem Confirm Modal
          </button>
          <button
        ...
        <div className="mt-5 grid md:grid-cols-2 gap-4">
          ...
          <div className="border-2 border-dashed border-border rounded-2xl p-4 flex items-center justify-center bg-muted/20">
            <div className="bg-white rounded-2xl p-6 max-w-xs w-full shadow-lg">
              ...
              <p className="text-xs text-muted-foreground text-center">
                Dịch vụ "Sửa điện" sẽ bị xóa vĩnh viễn. Không thể hoàn tác.
              </p>
              <div className="flex gap-3 mt-5">
                <button className="flex-1 py-2 border border-border rounded-xl text-sm font-semibold hover:bg-muted transition-colors">
                  Hủy
                </button>
                <button className="flex-1 py-2 bg-red-600 text-white rounded-xl text-sm font-semibold">
                  Xóa
                </button>
              </div>
            </div>
          ...
          <div className="border-2 border-dashed border-border rounded-2xl p-4 flex items-center justify-center bg-muted/20">
            <div className="bg-white rounded-2xl p-6 max-w-xs w-full shadow-lg text-center">
              ...
              <p className="text-xs text-muted-foreground mb-4">
                Mã đặt lịch: <span className="font-bold text-foreground">#BK001</span>
              </p>
              <button className="w-full py-2.5 bg-blue-600 text-white rounded-xl text-sm font-bold">
                Xem đặt lịch
              </button>
            </div>
          </div>
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-muted-foreground text-center mb-6">
              Tài khoản Trần Minh Khoa sẽ bị xóa vĩnh viễn. Không thể hoàn tác.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setShowConfirm(false)}
          ...
          >
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  className="w-full bg-muted px-4 py-2.5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-600"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select className="w-full bg-muted px-3 py-2.5 rounded-xl text-sm focus:outline-none">
                    <option>Điện</option>
                    <option>Nước</option>
                  </select>
                </div>
                <div>
                  ...
                  <input
                    className="w-full bg-muted px-4 py-2.5 rounded-xl text-sm focus:outline-none"
                    placeholder="150,000"
                  />
                </div>
              </div>
    ...
    <div className="space-y-8">
      ...
      <Card>
              ...
              {Array.from({ length: total }, (_, i) => i + 1)
                ...
                .map((p, i) => (
                  <button
                    ...
                    disabled={p === "..."}
                    className={`w-8 h-8 rounded-lg text-xs font-semibold transition-colors ${p === page ? "bg-blue-600 text-white" : p === "..." ? "cursor-default text-muted-foreground" : "border border-border hover:bg-muted"}`}
                  >
                    {p}
                  </button>
                ))}
        ...
        <SubSection title="Simple Prev / Next">
          <div className="flex gap-3">
            <button className="flex items-center gap-1.5 px-4 py-2 border border-border rounded-xl text-sm font-semibold hover:bg-muted transition-colors">
              <ChevronLeft className="w-4 h-4" />
              Trước
            </button>
            <button className="flex items-center gap-1.5 px-4 py-2 bg-blue-600 text-white rounded-xl text-sm font-semibold hover:bg-blue-700 transition-colors">
              Tiếp
              <ChevronRight className="w-4 h-4" />
            </button>
          </div>
        ...
        <div className="bg-white border-b border-border flex items-center h-14 px-4 gap-3">
          ...
          <div className="flex items-center gap-0.5 flex-1">
            {["Trang chủ", "Dịch vụ", "Lịch đặt", "Hồ sơ"].map((n, i) => (
              <button
                key={n}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${i === 0 ? "bg-accent text-blue-600" : "text-muted-foreground hover:bg-muted"}`}
              >
                {n}
              </button>
            ))}
        ...
        <div className="flex h-56">
          <div className="w-48 bg-white border-r border-border flex flex-col py-3 px-2 space-y-0.5">
            {[
              ...
              <button
                key={item.label}
                className={`flex items-center gap-2.5 px-2.5 py-2 rounded-xl text-xs font-semibold transition-colors ${item.active ? "bg-blue-600 text-white" : "text-muted-foreground hover:bg-muted"}`}
              >
          ...
          <div className="flex-1 bg-muted/30 flex items-center justify-center text-xs text-muted-foreground">
            Content area
          </div>
        </div>
      </Card>
      ...
      <Card className="p-0 overflow-hidden max-w-xs">
        ...
        <div className="bg-white border-t border-border flex items-center px-2 py-2">
          {[
            ...
            { icon: Search, label: "Dịch vụ", active: false },
            { icon: FileText, label: "Lịch đặt", active: false },
            { icon: User, label: "Hồ sơ", active: false },
          ].map((item) => (
            <button
              key={item.label}
              className={`flex-1 flex flex-col items-center gap-1 py-1 ${item.active ? "text-blue-600" : "text-muted-foreground"}`}
            >
...
function SectionCharts() {
  ...
  const pieData = [
    { label: "Điện", pct: 35, color: "bg-amber-400" },
    { label: "Dọn dẹp", pct: 28, color: "bg-green-400" },
    { label: "Điều hòa", pct: 18, color: "bg-blue-400" },
    { label: "Nước", pct: 12, color: "bg-cyan-400" },
    { label: "Khác", pct: 7, color: "bg-purple-400" },
  ];
      ...
      <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="flex items-end gap-1.5 h-32">
            {barData.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                ...
                <div
                  className={`w-full rounded-t-lg ${i === barData.length - 1 ? "bg-blue-600" : "bg-blue-100"}`}
                  style={{ height: `${(d.v / max) * 80}px` }}
                />
        ...
        <Card>
          <p className="text-sm font-bold text-foreground mb-4">
            Horizontal Bar — Theo loại dịch vụ
          </p>
          <div className="space-y-3">
            {pieData.map((item) => (
              <div key={item.label}>
  ...
  const toasts = [
    {
      ...
      body: "Thợ sẽ liên hệ trong 5 phút.",
    },
    {
      type: "error",
      icon: AlertCircle,
      ...
      body: "Vui lòng kiểm tra thông tin thẻ.",
    },
    {
      type: "warning",
      icon: AlertCircle,
    ...
    },
    {
      ...
      title: "Cập nhật mới",
      body: "Phiên bản 2.6 đã sẵn sàng.",
    },
  ];
  return (
    <div className="space-y-8">
      ...
      <Card>
        <div className="space-y-3 max-w-sm">
          {toasts.map((t) => (
            <div
              key={t.type}
              className={`flex items-start gap-3 px-4 py-3 rounded-xl border-l-4 shadow-sm ${t.color}`}
            >
...
function SectionStates() {
  return (
    <div className="space-y-8">
      <SectionTitle
        title="System States"
        subtitle="Loading, Empty, No Data, Error, Success — required for every list screen"
      />
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {[
          ...
          },
          {
            ...
            content: (
              <div className="flex flex-col items-center gap-3">
                ...
                <button className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-bold">
          ...
          },
          {
            ...
            content: (
              <div className="flex flex-col items-center gap-3">
                ...
                <button className="flex items-center gap-1.5 px-3 py-1.5 border border-border rounded-lg text-xs font-semibold hover:bg-muted transition-colors">
          ...
          <div
            key={state.label}
            className="border-2 border-dashed border-border rounded-2xl p-6 flex flex-col items-center gap-3"
          >
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider self-start">
              {state.label}
            </p>
            {state.content}
          </div>
...
function SectionForms() {
  return (
    <div className="space-y-8">
      <SectionTitle
        title="Form Patterns (CRUD)"
        subtitle="Standard form layouts — Create, Edit, Delete confirmation"
      />
      <div className="grid md:grid-cols-2 gap-6">
        <Card>
          ...
          <div className="space-y-3">
            <div>
              ...
              <input
                className="w-full bg-muted px-4 py-2.5 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-600"
                placeholder="VD: Sửa điện dân dụng"
              />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
            ...
            <div>
              ...
              <textarea
                rows={2}
                className="w-full bg-muted px-4 py-2.5 rounded-xl text-sm focus:outline-none resize-none"
            ...
            <div className="flex items-center justify-between pt-1">
              ...
              <div className="flex gap-2">
                <button className="px-3 py-1.5 border border-border rounded-lg text-xs font-semibold hover:bg-muted transition-colors">
                  Hủy
                </button>
            ...
            {[
              {
                ...
                desc: "POST /api/services",
              },
              {
                label: "Read / List",
                icon: Search,
                cls: "bg-muted text-foreground",
                desc: "GET /api/services?page=1&limit=10",
              },
              {
                label: "Read / Detail",
                icon: Eye,
              ...
              },
              {
                label: "Update",
                icon: Edit3,
                cls: "border border-border text-foreground",
                desc: "PUT /api/services/:id",
              },
              {
                label: "Delete",
                icon: Trash2,
                cls: "bg-red-50 text-red-600",
                desc: "DELETE /api/services/:id",
              },
              {
                label: "Approve / Reject",
                icon: Check,
                cls: "bg-green-50 text-green-700",
                desc: "PATCH /api/providers/:id/status",
              },
            ].map((a) => (
              <div key={a.label} className="flex items-center gap-3 p-2 bg-muted rounded-xl">
                <button
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold flex-shrink-0 ${a.cls}`}
                >
  ...
  return (
    <div className="space-y-8">
      <SectionTitle
        ...
        subtitle="Full CRUD table — anatomy & REST API mapping"
      />
      <Card>
          ...
          {[
            {
              ...
              content: "Page title + description + [+ Thêm mới] button",
            },
            {
              zone: "2 · Stats Strip",
              cls: "bg-green-50 border border-green-200",
              content: "3–4 KPI mini cards (total, active, pending, etc.)",
            },
            {
              zone: "3 · Toolbar",
              cls: "bg-amber-50 border border-amber-200",
              content: "SearchBar + Filter dropdowns + [Sort] + [Export]",
            },
            {
              zone: "4 · Table Head",
              cls: "bg-purple-50 border border-purple-200",
              content: "Checkbox (bulk) · Columns with sort icons · Actions",
            },
            {
              zone: "5 · Table Body",
              cls: "bg-slate-50 border border-slate-200",
              content:
                "Rows: avatar + name · badge · numbers · status badge · [Chi tiết][Sửa][Xóa]",
            },
            {
              zone: "6 · Pagination",
              cls: "bg-orange-50 border border-orange-200",
              content: "Record range · Page buttons · rows-per-page select",
            },
            {
              zone: "7 · Detail Panel",
              cls: "bg-cyan-50 border border-cyan-200",
              content: "Slide-in right panel with all fields + action buttons",
            },
            {
              zone: "8 · Confirm Modal",
              cls: "bg-red-50 border border-red-200",
              content: "Overlay modal: icon + title + message + [Hủy][Xác nhận]",
            },
          ].map((z, i) => (
    ...
    <div className="flex h-full bg-background" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="w-56 flex-shrink-0 bg-white border-r border-border flex flex-col overflow-hidden">
        ...
        <div className="h-14 flex items-center justify-between px-4 border-b border-border flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="w-7 h-7 bg-muted rounded-lg flex items-center justify-center hover:bg-accent transition-colors"
          >
            <X className="w-3.5 h-3.5 text-muted-foreground" />
          </button>
        </div>
