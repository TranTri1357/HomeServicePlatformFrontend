  ...
  return (
    <div className="flex flex-col h-full">
      <TopBar
        ...
        actions={
          <button onClick={markAllRead} className="text-blue-600 text-xs font-semibold">
            Đọc tất cả
          </button>
        }
      />
        ...
        {items.map((notif) => {
          ...
          return (
            <button
              key={notif.id}
              onClick={() =>
                setItems(items.map((i) => (i.id === notif.id ? { ...i, read: true } : i)))
              }
            ...
            >
              ...
              <div className="flex-1 text-left">
                ...
                <p className="text-[11px] text-muted-foreground mt-1.5 font-medium">{notif.time}</p>
              </div>
            </button>
          );
        })}
