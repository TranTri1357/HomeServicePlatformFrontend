import { Star, Edit3, Camera, Phone, MessageCircle, MapPin, User, LogOut } from "lucide-react";
      ...
      <div className="bg-gradient-to-br from-slate-800 to-slate-900 px-4 pt-6 pb-12">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">{tech.skill}</p>
            <div className="flex items-center gap-2 mt-1">
              <div className="flex items-center gap-1">
                <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-400" />
                <span className="text-white text-sm font-semibold">{tech.rating}</span>
              </div>
              <span className="text-slate-400 text-sm">{tech.jobs} công việc</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm grid grid-cols-3 divide-x divide-border">
          {[
            ...
            <div key={l} className="flex flex-col items-center gap-1 px-3">
              <span className="text-xl font-extrabold text-blue-600">{v}</span>
