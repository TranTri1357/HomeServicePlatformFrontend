...
export function Orders() {
  ...
  const [selected, setSelected] = useState<(typeof adminOrdersList)[0] | null>(null);
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Theo dõi và quản lý tất cả đơn đặt lịch</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {[
          ...
          {
            label: "Chờ xác nhận",
            value: adminOrdersList.filter((o) => o.status === "pending").length,
            color: "text-amber-600",
          },
          {
            label: "Đang làm",
            value: adminOrdersList.filter((o) => o.status === "in_progress").length,
            color: "text-purple-600",
          },
          {
            label: "Hoàn thành",
            value: adminOrdersList.filter((o) => o.status === "completed").length,
            color: "text-green-600",
          },
          {
            label: "Đã hủy",
            value: adminOrdersList.filter((o) => o.status === "cancelled").length,
            color: "text-red-600",
          },
        ].map((s) => (
            ...
            <tbody className="divide-y divide-border">
              {paged.map((o) => (
                <tr key={o.id} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm text-foreground">{o.provider}</td>
                  <td className="px-4 py-3 text-sm text-foreground">{o.service}</td>
                  <td className="px-4 py-3 text-xs text-muted-foreground">
                    {o.date}
                    <br />
                    {o.time}
                  </td>
                  <td className="px-4 py-3 text-sm font-bold text-foreground">{o.price}đ</td>
                  <td className="px-4 py-3 text-sm font-semibold text-green-600">
                    {o.commission}đ
                  </td>
                  <td className="px-4 py-3">
                    <Badge status={o.status as keyof typeof statusConfig} />
                  </td>
                  <td className="px-4 py-3">
        ...
        >
          ...
          >
            <div className="sticky top-0 bg-white border-b border-border px-5 py-4 flex items-center justify-between">
              <h3 className="font-bold text-foreground">Chi tiết đơn {selected.id}</h3>
              <button
                onClick={() => setSelected(null)}
            ...
            <div className="p-5 space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-2xl font-extrabold text-blue-600">{selected.price}đ</span>
                <Badge status={selected.status as keyof typeof statusConfig} />
              </div>
              {[
      ...
      {confirm.show && confirm.item && (
        <ConfirmModal
          ...
          onConfirm={() => setConfirm({ show: false, item: null })}
          onCancel={() => setConfirm({ show: false, item: null })}
        />
      )}
    </div>
  );
