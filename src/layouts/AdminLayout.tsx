...
export function AdminLayout({ currentScreen, onNavigate, onLogout, children }: AdminLayoutProps) {
  ...
  const Sidebar = ({ mobile = false }: { mobile?: boolean }) => (
    <div
      className={`${mobile ? "w-64" : sidebarOpen ? "w-60" : "w-16"} flex flex-col h-full bg-white border-r border-border transition-all duration-200`}
    >
      <div className="h-16 flex items-center px-4 border-b border-border gap-3 flex-shrink-0">
        ...
        {(sidebarOpen || mobile) && (
          <div>
            ...
            <span className="ml-1.5 text-[10px] font-bold bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded">
              Admin
            </span>
          </div>
        )}
